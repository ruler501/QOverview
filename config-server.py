...
def _get_background():
    ...
    if options.get('background-image', '{DESKTOP_WALLPAPER}') \
            != '{DESKTOP_WALLPAPER}':
    ...
    if desktop_env in ['gnome', 'unity', 'cinnamon', 'pantheon', 'mate']:
        ...
        try:
            proc = sp.check_output(['gsettings', 'get', SCHEMA, KEY])\
                     .decode('utf-8')
        except sp.CalledProcessError:
            proc = sp\
                .check_output(['mateconftool-2', '-t', 'string', '--get',
            ...
            for line in f:
                if line.startswith('theme_path'):
                    ...
                    awesome_theme = awesome_theme[
                        len(awesome_theme) - 1].strip().replace('"', '')\
                                               .replace("'", '')
            ...
            for line in f:
                if line.startswith('theme.wallpaper'):
                    ...
                    awesome_wallpaper = awesome_wallpaper[
                        len(awesome_wallpaper) - 1].strip().replace('"', '')\
                        .replace("'", '')
...
def _desktop_entry_locate(desktop_file_name):
    ...
    for path in search_paths:
        for f in os.listdir(path):
            # reverse, to be able to split on last '.',
            # then get the last item, reversed
            # all this so that we can split on the last '.'
            if f[::-1].split('.', 1)[-1][::-1] == desktop_file_name \
                    or f == desktop_file_name:
...
def _desktop_entry_execute(desktop_file, files=None,
                           return_cmd=False, background=True):
...
def _get_icon(icon_name, categories=['apps', 'actions', 'preferences', ]):
    ...
    search_paths = [os.path.expanduser('~/.icons'),
    ...
    if options.get('icon-theme', '{SYSTEM_THEME}') != '{SYSTEM_THEME}':
        ...
        if desktop_env in ['gnome', 'pantheon', 'cinnamon', 'budgie']:
            theme = sp.check_output(['gsettings', 'get',
                                     'org.gnome.desktop.interface',
                                     'icon-theme']).decode('utf-8')

        elif desktop_env == 'kde':
            theme = sp.check_output(['kreadconfig5', '--group', 'Icons',
                                     '--key', 'Theme', '--default', 'breeze'])\
                      .decode('utf-8')

        elif desktop_env == 'xfce':
            theme = sp.check_output(['xfconf-query', '-c', 'xsettings',
                                     '-p', '/Net/IconThemeName'])

        else:
            print('Cannot detect system icon theme; falling back to hicolor.',
                  'Try setting an icon theme in the settings.')
            theme = 'hicolor'
    ...
    for path in search_paths:
        for cat in categories:
            for size in ['64', '48', '32', '24', '22', '12']:
                for ext in ['.svg', '.png']:
                    if theme == 'hicolor':
                        if os.path.exists(os.path.join(path, 'hicolor',
                                                       '{}x{}'.format(size,
                                                                      size),
                                                       cat, icon_name + ext)):
                            return os.path.join(path, 'hicolor',
                                                '{}x{}'.format(size, size),
                                                cat, icon_name + ext)

                    else:
                        if os.path.exists(os.path.join(path, theme, cat,
                                                       size, icon_name + ext)):
                            return os.path.join(path, theme, cat, size,
                                                icon_name + ext)

    # Let's try /usr/share/pixmaps and ~/.local/share/pixmaps

    for path in [os.path.expanduser('~/.local/share/pixmaps'),
                 '/usr/share/pixmaps']:
    ...
    if icon_name[::-1].split('.', 1)[0][::-1] in ['svg', 'png', 'xpm', 'jpg']:
        return _get_icon(icon_name[::-1].split('.', 1)[-1][::-1],
                         categories=categories)

    # Still no icon?
    # Fallback to included missing-icon.svg

    print('No icon found for {}; falling back to missing-icon.svg'
          .format(icon_name))
...
for path in search_paths:
    for entry in os.listdir(path):
        # some distros (eg. KDE Neon) keep .distrib files here
        if entry.endswith('.desktop'):
            if os.path.isfile(os.path.join(path, entry)):
                desktop_entries.append(entry[:-len('.desktop')])
...
class Service(dbus.service.Object):
    ...
    def run(self):
        ...
        bus_name = dbus.service.BusName("org.qoverview.config",
                                        dbus.SessionBus())
    ...
    @dbus.service.signal('org.qoverview.config.iface')
    def CloseInstances(self):
        print('Closing Instances')
